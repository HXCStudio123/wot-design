...
设置 `before-confirm` 函数，在用户点击`确定`按钮时，会执行 `before-confirm` 函数，并传入`value`(选中项 也就是当前选择的数组） 和 `resolve` 参数，可以对 `value` 进行校验，并通过 `resolve` 函数告知组件是否确定通过，`resolve` 接受1个 boolean 值，`resolve(true)` 表示选项通过，`resolve(false)` 表示选项不通过，不通过时不会关闭弹窗。

```html
<wd-select-picker label="展示格式化" v-model="value" :columns="columns" :before-confirm="beforeConfirm"></wd-select-picker>
...
```html
<wd-select-picker label="错误" v-model="value" :columns="columns" required></wd-select-picker>
```
...
```html
<wd-select-picker label="错误" v-model="value" :columns="columns" size="large"></wd-select-picker>
```
...
```html
<wd-select-picker label="错误" v-model="value" :columns="columns" align-right></wd-select-picker>
```
...
通过设置插槽 `content` 可以自定义弹出框内容，结合默认唤起插槽和方法`cancel` / `confirm` 使用可自定义picker。
...
<wd-select-picker @cancel="handleClose" @confirm="handleConfirm">
  ...
  <div slot="content" class="showdemo">
    <wd-input placeholder="请输入..." size="large" v-model="value" @change="changeInput" />
  </div>
</wd-select-picker>
...
| select-size | 设置picker内部的选项组尺寸大小 | string | 'large' | - |
