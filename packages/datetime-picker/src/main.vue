...
export default {
  ...
  methods: {
    ...
    columnDisabledRules (type, columns, cIndex, value, currentValue, boundary, ranges) {
      ...
      // const limit = type === 'start' ? currentValue : currentValue
    ...
    columnFormatter (originColumns, ranges) {
      const boundary = this.innerValue
      const mapColumns = (columns, type) => {
        ...
        return columns.map((column, cIndex) => {
          return column.values.map((value, index) => {
            const disabled = this.columnDisabledRules(type, columns, cIndex, value, this.innerValue, boundary, ranges)
            return {
